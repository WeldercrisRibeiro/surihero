        ...
        {content.items.map((item, idx) => {
          ...
          return (
            <div key={idx} className={`flex gap-4 p-4 rounded-3xl transition-all ${isHighlight ? "bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-400 dark:border-amber-600 shadow-md" : "bg-slate-50/50 dark:bg-slate-800/40 hover:bg-slate-100/50 dark:hover:bg-slate-800/60"}`}>
              <div className={`w-12 h-12 rounded-full shadow-sm flex items-center justify-center shrink-0 ${isHighlight ? "bg-amber-100 dark:bg-amber-900/50 text-amber-600 dark:text-amber-400" : "bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400"}`}>
                {item.icon}
              </div>
              <div>
