...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!email.trim().toLowerCase().endsWith("@chatbotmaker.io")) {
      toast.error("Acesso restrito. Use um e-mail @chatbotmaker.io.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userEmail = data.platformUser?.email || "";

        if (!userEmail.toLowerCase().endsWith("@chatbotmaker.io")) {
          toast.error("Acesso não autorizado para este domínio.");
          clearCredentials();
          return;
        }
        ...
        if (token) {
          saveCredentials({
            ...
            token: token,
            userName: data.platformUser?.name || "",
            userEmail: data.platformUser?.email || "",
          });
      ...
      // ✅ Sem mock token — erro real é mostrado ao usuário
      toast.error("Erro de conexão com o servidor. Verifique sua rede.");
    } finally {
      setLoading(false);
    }
  };
            ...
            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  type="email"
                  placeholder="email@chatbotmaker.io"
                  value={email}
